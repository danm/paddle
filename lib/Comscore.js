...
module.exports = class Comscore {
    ...
    continueDataStream() {
        ...
        if (self.daxConfig.maxlines) {
            self.options.qs.maxlines = self.daxConfig.maxlines;
        }
    ...
    startNewDataStream() {
        ...
        if (self.settings.maxlines) {
            self.options.qs.maxlines = self.settings.maxlines;
        }
    ...
    getDataStream() {
        ...
        rl.on('data', (line) => {
            ...
            const buffered = line.toString();
            if (typeof buffered === 'string') {
                const cols = buffered.split('\t');
