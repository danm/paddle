...
module.exports = class Comscore {
    ...
    start() {
        ...
        dax.site(self.daxConfig, self.site, c, (err, data) => {
            if (data === undefined || data.length === 0) {
                //beed to create a new data stream
                this.c.a('No Continue ID Found ' + self.site, 1, 'start');
                self.startNewDataStream();
            } else if (typeof data[0] === 'object' && data[0]['Continuation-Id']) {
                dax.continuation(self.daxConfig, self.site, data[0]['Continuation-Id'], c, (err, cont) => {
    ...
    continueDataStream() {
        let self = this;
        this.c.a('Continueing, setting continuation id ' + self.id, 1, 'startNewDataStream');
    ...
    startNewDataStream() {
        ...
        this.c.a('Creating new data stream ' + self.site, 1, 'startNewDataStream');
    ...
    getDataStream() {
        ...
        this.c.a('Recieving data from Comscore', 1, 'getDataStream ' + self.site);

        //Stream events
        upload.on('error', (error) => {
            this.c.a('Error uploading file ' + self.site, 3, 'getDataStream->error');
            this.error++;
            this.status = 'error';
            self.processAndWait();
        });
        ...
        upload.on('uploaded', (details) => {
            this.c.a('Uploaded File ' + self.site, 1, 'getDataStream->uploaded');
            this.status = 'Uploaded File';
            self.processAndWait();
        });

        stream.on('response', (res) => {
            this.count++;
            this.status = 'Respose Recieved';
            this.c.a('Response Recieved ' + self.site, 1, 'getDataStream->response');
            ...
            if (res.headers['x-cs-next-request-date'] && res.headers['x-cs-continuation-id']) {
                ...
                if (self.next.getFullYear() > 2010) {
                    ...
                    this.c.a('Next stream prepared', 1, 'getDataStream ' + self.site + ' ' + res.headers['x-cs-continuation-id']);
                    ...
                    //
                    this.c.a('Comscore date response was not about year 2000', 3, 'getDataStream ' + self.site);
                    self.status = 'error';
                }

            } else {

                this.c.a('Comscore response did not provide the correct headers', 3, 'getDataStream ' + self.site, res.headers);
        ...
        if (self.status === 'error') {
            ...
            self.timer = setInterval(() => {
                ...
                if (self.next <= now) {
                    ...
                    this.c.a('Timer complete Continuing Stream ' + self.site, 1, 'processAndWait');
                    self.continueDataStream();
                } else {
    ...
    wait(seconds) {
        ...
        if (self.error > 5) {
            this.c.a('Multilple errors on ' + self.site, 3, 'wait');
        }
