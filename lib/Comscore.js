...
Date.prototype.fileDate = function() {
    return this.getFullYear() + '-' + ('0' + (this.getMonth() + 1)).slice(-2) + '-' + ('0' + (this.getDate() + 1)).slice(-2) + '-' + ('0' + this.getHours()).slice(-2) + '-' + ('0' + this.getMinutes()).slice(-2);
};
    ...
    getDataStream() {
        ...
        upload.on('uploaded', (details) => {
            if (self.status === 'error') {
                self.c.a('The file has been uploaded but there was a problem with the header' + self.site, 2, 'getDataStream->uploaded');
            } else {
